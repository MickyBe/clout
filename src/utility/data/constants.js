import { baseUrl } from "../../config/url/api.url";
import moment from "moment";

export const TIME = { min: 0, max: 24 };

export const apiKey = "e3Gc3Zt4Ku";

export const RequestProcessType = {
  ACCEPT_REQUEST: "ACCEPT_REQUEST",
  REJECT_REQUEST: "REJECT_REQUEST",
  SEND_REQUEST: "SEND_REQUEST",
};

export const ChangeVisibilityType = {
  ACCEPT_VISIBLE: "ACCEPT_VISIBLE",
  REJECT_VISIBLE: "REJECT_VISIBLE",
};

export const ChangeGroupVisibilityType = {
  ACCEPT_VISIBLE: "ACCEPT_VISIBLE",
  REJECT_VISIBLE: "REJECT_VISIBLE",
};

export const GroupVisibilityTypes = {
  ONLY_FRIENDS: "ONLY_FRIENDS",
  REQUEST_TO_JOIN: "REQUEST_TO_JOIN",
  PUBLIC: "PUBLIC",
};

export const GroupRequestProcessType = {
  ACCEPT_GROUP_REQUEST: "ACCEPT_GROUP_REQUEST",
  REJECT_GROUP_REQUEST: "REJECT_GROUP_REQUEST",
};

export const ActivityDateType = {
  ONE_MONTH: "ONE_MONTH",
  TWO_WEEKS: "TWO_WEEKS",
  ONE_WEEK: "ONE_WEEK",
  TWO_DAYS: "TWO_DAYS",
  ONE_DAY: "ONE_DAY"
}

export const ActivityData = [
  {
    type: ActivityDateType.ONE_DAY,
    title: "24",
    subTitle: "HOURS",
    date: [moment().subtract(1, "day").startOf('day').toISOString(), moment().endOf("day").toISOString()]
  },
  {
    type: ActivityDateType.TWO_DAYS,
    title: "2",
    subTitle: "DAYS",
    date: [moment().subtract(2, "days").endOf("day").toISOString(), moment().toISOString(), ]
  },
  {
    type: ActivityDateType.ONE_WEEK,
    title: "1",
    subTitle: "WEEK",
    date: [moment().subtract(1, "week").endOf("day").toISOString(), moment().toISOString()]
  },
  {
    type: ActivityDateType.TWO_WEEKS,
    title: "2",
    subTitle: "WEEKS",
    date: [moment().subtract(2, "weeks").endOf("day").toISOString(), moment().toISOString(), ]
  },
  {
    type: ActivityDateType.ONE_MONTH,
    title: "1",
    subTitle: "MONTH",
    date: [moment().subtract(1, "month").endOf("day").toISOString(), moment().toISOString()]
  }
]

export const defaultImage =
  "https://www.pngitem.com/pimgs/m/150-1503945_transparent-user-png-default-user-image-png-png.png";

export const imageDir = (img) =>
  img
    ? `${baseUrl}/${img}`
    : "data:image/png;base64,";
